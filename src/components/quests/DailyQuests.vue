<template>
  <main class="flex md:justify-center items-center flex-col relative">
    ...
    <div class="mt-5 md:w-[350px] w-full md:px-2 px-9 text-2xl md:text-lg font-light ">
      ...
      <p :class="dailyQuestCompleted ? 'text-green-600 text-shadow-green' : 'text-red-600 text-shadow-red'"
        class="my-5 uppercase">
        {{ dailyQuestCompleted ? 'Daily quest completed' : 'Daily quest not yet completed' }}
      </p>
    </div>
    <div :class="dailyQuestCompleted ? 'from-green-600 to-green-950' : 'from-red-600 to-red-950'"
...
import InputCheckbox from '../details/Inputs.vue'
...
const checked = computed(() => ['Pushups', 'Sit-ups', 'Squats', 'Running'].every(exercise => checkedExercise.value.includes(exercise))
);

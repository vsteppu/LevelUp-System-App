<template>
  <div v-if="loading" class="bg-neutral-950 w-full top-16 bottom-0 absolute z-10 flex justify-center items-center ">
  ...
  <main class="flex md:justify-center md:items-center flex-col bg-gradient-to-tl from-[#222222] to-neutral-900 bg-[#222222] md:mx-40 relative">
    ...
    <div v-if="setName === undefined" class="font-light flex md:px-0 px-6 justify-between md:w-[450px]">
      <input v-model="playerName" @keyup.enter="setUserValues(playerName)" placeholder="type your name"
        class="my-9 flex flex-grow mr-[2px] w-44 bg-neutral-700 px-4 py-1">
      <button @click="setUserValues(playerName)" class="my-9 flex bg-neutral-700 px-2 py-1">Confirm</button>
    </div>
    <p v-if="setName === undefined" class="text-green-600 text-xl mb-9 font-light flex justify-center items-center">Type
      your name in the field to start the game</p>
    <div class="flex md:justify-between justify-between my-4 w-full md:w-[650px] md:px-2 px-6 text-xl md:text-xl uppercase">
        ...
        <div v-if="deleteAccountPopup"
          ...
          <div class="flex items-center justify-around w-full px-9 text-lg pb-4 ">
            <button @click="deleteAccountPopup = false"
              class="w-40 h-12 bg-neutral-800  border-[1px] border-neutral-900">Cancel</button>
...
const userStatus = async () => {
  ...
  if (user.user_metadata.daily_quests) { dailyQuest.value = 'ACCOMPLISH' } else { dailyQuest.value = 'uncompleated' }
